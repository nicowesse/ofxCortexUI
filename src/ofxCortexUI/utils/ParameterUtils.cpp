...
std::vector<shared_ptr<ofxCortex::ui::View> > ParameterUtils::createViewsFromParameter(ofAbstractParameter & param)
{
  ...
  cout << "createViewsFromParameter(): Type = " << type << endl;
  cout << "UnitParameter<float> type = " << typeid(UnitParameter<float>).name() << endl;
  ...
  else if (type == typeid(UnitParameter<float>).name())
  {
    cout << "Cast to UnitParameter" << endl;
    views.push_back(Slider<float>::create(param.cast<float>()));
  }
